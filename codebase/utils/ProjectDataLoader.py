...
def loadTestData_matpip(featureDir, fixed_prot_id, mut_prot_id_lst):
    trainSets  = []; saves = []; predFNames = []
    testSets = [(fixed_prot_id, mut_prot_id) for mut_prot_id in mut_prot_id_lst]
    testSets = [np.asarray(testSets)]
    predFNames = None
    return trainSets, testSets, saves, predFNames, featureDir
