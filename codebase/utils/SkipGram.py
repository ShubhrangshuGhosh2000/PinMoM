...
def SkipGram(fastas, fileName, windowSize=7, negativeSize=5, hiddenSize=7, corpusSmall=True, numEpochs=1,groupings=None,groupLen=1,sorting=False, flip=False,excludeSame=False, preTrained=False, deviceType='cpu',fullGPU=False,saveModel=True,preCompute=True,softMax=False
             ,skipgrm_lookup_prsnt=False):
    
    # For SkipGram feature extraction
    # ================================
    # IN THE PATH "dataset/preproc_data/derived_feat/" PUT THE FOLLOWING FILE: SkipGramAA7H5_skipgram.model
    ...
    if (not skipgrm_lookup_prsnt):
        ...
        skipRunner = SkipGramNetworkRunner(skipModel, deviceType=deviceType)
        ...
        # contains 1st part of SkipGramAA7H5.encode file
        for i in range(0,corpusSize):
            lst = vals[i,:].tolist()
            lookupMatrix.append(lst)
        lookupMatrix = torch.tensor(lookupMatrix).long()
    # end of if block: if (not skipgrm_lookup_prsnt):
    
    #lookup for protein name to row index mapping
    proteinNameMapping = {}  # contains 2nd part of SkipGramAA7H5.encode file
    ...
    for item in parsedData[1:]:
        name = item[0]
        aaIdx = torch.tensor(item[1].numpy()).long()
    ...
    skipgram_feat_dict = {'lookupMatrix': lookupMatrix, 'proteinNameMapping': proteinNameMapping
                          , 'aaIdxs': aaIdxs}
    
    return skipgram_feat_dict
