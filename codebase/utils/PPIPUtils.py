...
def get_top_level_directories(directory):
    top_level_directories = [d for d in os.listdir(directory) if os.path.isdir(os.path.join(directory, d))]
...
def execute_unix_command(command):
    try:
        ...
        # retrieve the output and any errors
        out = result.stdout.decode(); err = result.stderr.decode()
