...
class MatP2ipNetworkModule(GenericNetworkModule):
    ...
    def loadFeatureData_matpip(self, fixed_prot_id, feature_data):
        plm_feat_dict, man_2d_feat_dict_prot, man_1d_feat_dict_prot = feature_data
        dataLookupSkip, dataMatrixSkip = self.loadEncodingFileWithPadding_pd(fixed_prot_id, man_2d_feat_dict_prot, self.maxProteinLength)
        dataLookupLabelEncode, dataMatrixLabelEncode = self.loadLabelEncodingFileWithPadding_pd(fixed_prot_id, man_2d_feat_dict_prot, self.maxProteinLength)
        # creating the conslidated pssm_dict, blosum62_dict, man_1d_feat_dict dictionary
        pssm_dict, blosum62_dict= {}, {}
        for prot_id, indiv_prot_man_2d_feat_dict in man_2d_feat_dict_prot.items():
            pssm_dict[prot_id] = indiv_prot_man_2d_feat_dict['PSSM'][prot_id]
            blosum62_dict[prot_id] = indiv_prot_man_2d_feat_dict['Blosum62'][prot_id]

        allProteinsSet = set(list(dataLookupSkip.keys())) & set(list(dataLookupLabelEncode.keys()))
        allProteinsList = list(plm_feat_dict.keys())
        ...
        for item in allProteinsSet:
            ...
            # extract tl-based 1d embeddings
            # tl_1d_embedd_tensor = torch.from_numpy(plm_feat_dict[item]['seq_feat'])
            tl_1d_embedd_tensor = plm_feat_dict[item]
            # extract other manual 1d embeddings 
            # seq_manual_feat_dict = DS_seq_manual_feat_dict[item]['seq_manual_feat_dict']
            seq_manual_feat_dict = man_1d_feat_dict_prot[item]
            other_man_feat_lst = seq_manual_feat_dict['AC30'] + seq_manual_feat_dict['PSAAC15'] + seq_manual_feat_dict['ConjointTriad'] \
                                + seq_manual_feat_dict['LD10_CTD_ConjointTriad_C'] + seq_manual_feat_dict['LD10_CTD_ConjointTriad_T'] \
                                + seq_manual_feat_dict['LD10_CTD_ConjointTriad_D']
            other_man_feat_arr = np.array(other_man_feat_lst)
            ...
            # print('aux_oneDencodingsize: ' + str(aux_1d_tensor.shape[0]))  # important print statement as its output will be used above
            self.oneDdataMatrix[self.dataLookup[item]] = aux_1d_tensor
        # end of for loop: for item in allProteinsSet:
