...
from proc.mat_p2ip_pd.mat_p2ip_pd_original_pair_only import run_prot_design_simuln
...
def trigger_experiment(root_path = './', user_input_dict=dict()):
    ...
    def_prob_threshold = 0.97  # default value
...
if __name__ == '__main__':
    ...
    dim_prot_complx_nm_lst_excluded = dim_prot_complx_nm_lst_done
    ...
    user_input_dict['num_of_itr_lst'] = [1]  #
    user_input_dict['batch_size'] = 1  # batch_size = 5 for Full length and 2 for interface;
    user_input_dict['batch_dump_interval'] = 1  # 5000 for batch size of 1; 3000 for batch size of 3 (or when use_prot_unit=True); 2000 for batch_size of 5
    user_input_dict['use_prot_unit'] = False  #

    # ###################################################### #
    user_input_dict['percent_len_for_calc_mut_pts_lst'] = [-1]   #
    # ###################################################### #
    ...
    # ###################################################### #
    user_input_dict['result_dump_dir'] = os.path.join(root_path, f'dataset/proc_data/result_dump_orig_pair_thorough')
    # ###################################################### #
