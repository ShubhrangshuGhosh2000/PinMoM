...
def perform_action(state, action):
    ...
    # Current episode is done if best_stability_score < 0.5
    done = True if(best_stability_score < 0.5) else False
    return best_next_state, best_stability_score, done


def compute_reward(current_stability):
    """Compute the reward based on the current_stability
    """
...
def train_dqn_agent(agent, initial_sequence, num_episodes, max_steps, convergence_threshold, patience):
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            next_state, current_stability, done = perform_action(state, action)
            reward = compute_reward(current_stability)
            ...
            if loss is not None:
                episode_loss += loss
            if done:
                agent.reset_epsilon()  # Reset epsilon when current episode is done
                break 
