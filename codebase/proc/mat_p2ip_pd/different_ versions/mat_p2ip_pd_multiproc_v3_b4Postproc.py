...
def run_prot_design_simuln(**kwargs):
    ...
    for arg_name, arg_value in kwargs.items():
        if((arg_value is None) and (arg_name not in ['temperature_scheduler', 'resource_monitor'])):
    ...
    num_of_mut_pts = kwargs.get('num_of_mut_pts'); exec_mode_type = kwargs.get('exec_mode_type'); plm_file_location = kwargs.get('plm_file_location')
    ...
    psiblast_exec_path = kwargs.get('psiblast_exec_path'); pdb_file_location = kwargs.get('pdb_file_location')
    ...
    # seq_first_chain_mapping contains unique sequences as keys and their corresponding first encountered chain names as values.
    seq_first_chain_mapping = prot_design_util.extract_chain_sequences(dim_prot_complx_nm, pdb_file_location)
    ...
    chain_sequences_dict = {chain_name: sequence for sequence, chain_name in seq_first_chain_mapping.items()}
    ...
    if(use_prot_unit):
        print(f'As use_prot_unit is True, calculating PU for chains ...')
        # estimate num_pu
    ...
    # ################ for unique_chain_name_tuple_idx in range(1,len(unique_chain_name_tuples_lst)): ################ TEMP CODE FOR AD-HOC TESTING
        ...
        print('\n################# Processing batch size- Start\n')
        # Sets batch_size as it depends on the value of use_prot_unit (True/False). If 'use_prot_unit' is True, 
        # then 'batch_size' input argument value is ignored and batch_size is set to the number of PU.
        if(use_prot_unit):
            print("**** As use_prot_unit is True, 'batch_size' input argument value is ignored and batch_size is set to the number of PU.")
            # set batch_size = num_pu for mut_prot_id
            batch_size = len(chain_nm_pu_dict[mut_prot_id])
            print(f'latest batch_size = num_pu for mut_prot_id ({mut_prot_id}) = {batch_size}')

        # check for num_of_itr to be a multiple of batch_size
        if(num_of_itr % batch_size != 0):
            print(f'**** Number of iterations ({num_of_itr}) is not a multiple of batch size ({batch_size}), so\
                  \n reducing the number of iterations to be exact multiple of batch size')
            num_of_itr = batch_size * int(num_of_itr / batch_size)
            print(f'modified num_of_itr = {num_of_itr}')
        # derive the number of batches
        num_of_batches = int(num_of_itr / batch_size)
        print('\n################# Processing batch size- End\n')

        print('\n################# Creating fresh result_dump_dir- Start\n')
        ...
        iter_spec_result_dump_dir = os.path.join(result_dump_dir, f'complex_{dim_prot_complx_nm}/fixed_{fixed_prot_id}_mut_{mut_prot_id}'
                                                 , f'res_totItr{num_of_itr}_batchSz{batch_size}_mutPts{num_of_mut_pts}_pu{use_prot_unit}')
        # if the result_dump_dir already exists, remove it as every run will be a fresh run
        if os.path.exists(iter_spec_result_dump_dir):
            print(f"Results dir '{iter_spec_result_dump_dir}' already exists... DELETE AND RECREATE")
            shutil.rmtree(iter_spec_result_dump_dir, ignore_errors=False, onerror=None)
        # create iter_spec_result_dump_dir
        os.makedirs(iter_spec_result_dump_dir)
        ...
        # initialize following early stopping criteria variables
        earlyStoppingCriteria_inst.best_prob = -1
        earlyStoppingCriteria_inst.no_improvement_count = 0
        ...
        for batch_idx in range(num_of_batches):
            ...
            for itr in range(mod_batch_size):
                ...
                # will be based on the use_prot_unit flag.

                # Consider all possible combinations of use_prot_unit (True/False), batch_size, num_of_mut_pts as follows:
                # Case-1: use_prot_unit = False; batch_size = 1; num_of_mut_pts = n (n> =1) ==> 
                ...
                # 
                # Case-2: use_prot_unit = False; batch_size = b (b >1); num_of_mut_pts = n (n >=1) ==> 
                # In each iteration, b (>1) cadidate mutated protein sequences will be generated (may be in parallel) from the prv_best_
                # mutated_prot_seq and each mutated candidate will have n (>=1) mutation(s) within the range of entire prv_best_mutated_prot_seq.
                # 
                # Case-3: use_prot_unit = True; batch_size = b (b >=1); num_of_mut_pts = n (n>=1) ==> In this case, user given batch_size 
            ...
            # if temperatureScheduler_inst is not None, then use temperature scheduler to obtain the temperature to be used in Metropolis Criteria (if required).
            ...
            if(temperatureScheduler_inst != None):
                t_mcmc = temperatureScheduler_inst.adjust_temperature(p_i)
            ...
            if(p_stable_changed):
                ...
                early_stop, early_stop_crit = earlyStoppingCriteria_inst.check_early_stopping(p_i)
            ...
            if((batch_idx + 1) % batch_dump_interval == 0):
                if(len(batch_out_dict_lst) > 0):  # check to see if batch_out_dict_lst is not empty, then only save it
                    ...
                    if(resourceMonitor_inst != None):  # resource usage monitoring is enabled
                        t10_1 = time.time()
                        resourceMonitor_inst.monitor_peak_ram_usage()
                        resourceMonitor_inst.monitor_peak_gpu_memory_usage()
                        t10_2 = time.time()
        ...
        if(early_stop):
            if(early_stop_crit == 'NIM'):
                end_condition = f'Early stopping - Prob Not Improved for {earlyStoppingCriteria_inst.stopping_patience} itr'
            elif(early_stop_crit == 'THC'):
                end_condition = f'Early stopping - Prob Threshold ({earlyStoppingCriteria_inst.prob_threshold}) crossed'
        ...
        print(f'############# Populating miscellaneous information - Start')
        ...
        misc_info_lst.append('PU_number'); misc_info_val_lst.append(batch_size if(use_prot_unit) else -1)
        ...
        misc_info_lst.append('peak_ram_usage_in_GB'); misc_info_val_lst.append(round(resourceMonitor_inst.peak_ram_usage / (1024 * 1024 * 1024.0), 3))
        # store peak_gpu_usage
        misc_info_lst.append('peak_gpu_usage_in_GB'); misc_info_val_lst.append(round(resourceMonitor_inst.peak_gpu_usage / 1024.0, 3))
