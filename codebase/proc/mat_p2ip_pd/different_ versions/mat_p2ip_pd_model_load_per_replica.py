...
def run_prot_design_simuln(**kwargs):
    ...
    # ################ for unique_chain_name_tuple_idx in range(1,len(unique_chain_name_tuples_lst)): ################ TEMP CODE FOR AD-HOC TESTING
        ...
        man_2d_feat_dict_prot_fixed = feat_engg_manual_main_pd.extract_prot_seq_2D_manual_feat(fix_mut_prot_id=fix_mut_prot_id, folderName=featureDir, fastas=fixed_prot_fastas, skipgrm_lookup_prsnt = False, use_psiblast_for_pssm=use_psiblast_for_pssm, psiblast_exec_path=psiblast_exec_path
        ...
        for repl_indx in range(num_of_replica_remc):
            ...
            indiv_replica_arg_dict['num_of_batches'] = local_step_size
        ...
        print(f'\n #############################################################################################')
        print(f'{fix_mut_prot_id_tag} ##### For REMC, in our implementation, local_step_size = batch_dump_interval = num_of_batches of size 1 = {local_step_size}')
        print(f'#############################################################################################\n')
...
def indiv_replica_work(indiv_replica_arg_dict):
    ...
    try:
        print(f'\n{fix_mut_prot_id_tag}################# One time model loading - Start\n')
        # initialize device
        device = torch.device(f'cuda:{cuda_index}')
        # load the protTrans PLM model -Start
        print('loading the protTrans PLM model -Start') 
        ...
        if(model_type == 'matds_hybrid'):
            ...
            alphabet = alphabets.Uniprot21()
            print('loaded D-SCRIPT model and pre-trained Bepler-Berger model')
        print(f'\n{fix_mut_prot_id_tag}################# One time model loading - End\n')

        print(f'\n{fix_mut_prot_id_tag} ##### For REMC, in our implementation, local_step_size = batch_dump_interval = num_of_batches of size 1 = {batch_dump_interval}')
        ...
        for batch_idx in range(num_of_batches):
            ...
            if(p_i > p_stable):
                ...
                if((del_p > 0.05) and (rand < math.exp(-(del_p / t_mcmc)))):  # Metropolis Criteria with del_p threshold
...
def extract_manual_feat_serially(fix_mut_prot_id, featureDir, mut_prot_fastas, use_psiblast_for_pssm, psiblast_exec_path, blosumMatrix, mut_prot_id_lst):
    ...
    for i in range(len(mut_prot_id_lst)):
        ...
        man_2d_feat_dict_prot_mut = feat_engg_manual_main_pd.extract_prot_seq_2D_manual_feat(fix_mut_prot_id=fix_mut_prot_id, folderName=featureDir, fastas=indiv_mut_prot_fastas, skipgrm_lookup_prsnt=skipgrm_lookup_prsnt, use_psiblast_for_pssm=use_psiblast_for_pssm, psiblast_exec_path=psiblast_exec_path, labelEncode_lookup_prsnt=labelEncode_lookup_prsnt, blosumMatrix=blosumMatrix)
