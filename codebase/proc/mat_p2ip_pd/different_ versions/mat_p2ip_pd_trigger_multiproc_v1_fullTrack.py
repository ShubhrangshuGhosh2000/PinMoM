...
def run_prot_design_simuln(root_path = './', fixed_prot_id = None, mut_prot_id = None, num_of_itr=300
                           ...
                           , batch_size=5, batch_dump_interval=5, cuda_index=0, psiblast_exec_path='./'):
    ...
    simuln_idx_lst, mut_pos_lst, aa_idx_lst = [], [], []
    ...
    for batch_idx in range(num_of_batches):
        ...
        mut_prot_fastas = []  # resets after every batch
        ppi_score_arr = np.empty((0,), dtype=float)  # resets after every batch
        ...
        if(p_stable_changed):
            ...
            batch_out_dict['simuln_idx'] = simuln_idx_lst[(batch_size * batch_idx) + max_val_index]
            batch_out_dict['mut_pos_lst'] = mut_pos_lst[(batch_size * batch_idx) + max_val_index]
            batch_out_dict['aa_idx_lst'] = aa_idx_lst[(batch_size * batch_idx) + max_val_index]
        ...
        if((batch_idx + 1) % batch_dump_interval == 0):
            if(len(batch_out_dict_lst) > 0):  # check to see if batch_out_dict_lst is not empty, then only save it
                ...
                batch_out_csv_nm = os.path.join(root_path, 'dataset/proc_data/result_dump', f'batchIdx_{batch_idx + 1 - batch_dump_interval}_{batch_idx}_batchSize_{batch_size}.csv')
    ...
    if(len(batch_out_dict_lst) > 0):
        ...
        batch_out_df = pd.DataFrame(batch_out_dict_lst)
        batch_out_csv_nm = os.path.join(root_path, 'dataset/proc_data/result_dump', f'batchIdx_{batch_dump_interval * (batch_idx // batch_dump_interval)}_{batch_idx}_batchSize_{batch_size}.csv')
        batch_out_df.to_csv(batch_out_csv_nm, index=False)
    # end of if block
...
if __name__ == '__main__':
    ...
    for num_of_itr in num_of_itr_lst:
        for num_of_mut_pts in num_of_mut_pts_lst:
            ...
            t22 = time.time()
            print(f'\n *********\n :: Total time for complete simulation execution :: time (in sec) for execution = {round(t22-t11, 3)} \n*********\n')
            # call for the gc
            gc.collect()
