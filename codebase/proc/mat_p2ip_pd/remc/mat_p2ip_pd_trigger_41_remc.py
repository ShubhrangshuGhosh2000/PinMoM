...
from proc.mat_p2ip_pd.remc.mat_p2ip_pd_remc import run_prot_design_simuln
...
def trigger_experiment(root_path = './', user_input_dict=dict()):
    ...
    # Number of batches after which result would be saved to disk during sumulation OR the result would be returned to the caller method in case of Replica-Exchange MC.
    # It is same as local_step_remc in case of Replica-Exchange MC
    ...
    def_use_resource_monitor = False  # default value
    ...
    # ############################ setting Replica-Exchange MC related inputs -Start ############################
    # Minimum temperature in the Boltzman temperature range used in Replica Exchange MC
    def_temp_min_remc = 0.02
    # check if the user_input_dict contains value for the key 'temp_min_remc'; 
    # otherwise, set it to the default value
    input_dict['temp_min_remc'] = user_input_dict.get('temp_min_remc', def_temp_min_remc)

    # Maximum temperature in the Boltzman temperature range used in Replica Exchange MC
    def_temp_max_remc = 0.2
    # check if the user_input_dict contains value for the key 'temp_max_remc'; 
    # otherwise, set it to the default value
    input_dict['temp_max_remc'] = user_input_dict.get('temp_max_remc', def_temp_max_remc)

    # Number of replicas to be used in Replica Exchange MC
    def_num_of_replica_remc = 10
    # check if the user_input_dict contains value for the key 'num_of_replica_remc'; 
    # otherwise, set it to the default value
    input_dict['num_of_replica_remc'] = user_input_dict.get('num_of_replica_remc', def_num_of_replica_remc)
    
    # Number of wrapper-step(s) after which REMC result would be saved in the disk.
    def_result_save_interval_remc = 2
    # check if the user_input_dict contains value for the key 'result_save_interval_remc'; 
    # otherwise, set it to the default value
    input_dict['result_save_interval_remc'] = user_input_dict.get('result_save_interval_remc', def_result_save_interval_remc)
    # ############################ setting Replica-Exchange MC related inputs -End ############################
...
if __name__ == '__main__':
    ...
    # ###################################################### #
    # ###################################################### #
    # dim_prot_complx_nm_lst_41_orig = ['1ZHH']
    ...
    user_input_dict['batch_dump_interval'] = 500  # Same as local_step_size in REMC
    ...
    # ###################################################### #
    user_input_dict['result_dump_dir'] = os.path.join(root_path, f'dataset/proc_data/result_dump_remc_fullLen_puFalse_mutPrcntLen10')
    # ###################################################### #
