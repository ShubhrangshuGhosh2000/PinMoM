...
def calc_accept_percent(root_path, iteration_tag, tot_replica=5, itr_per_replica=10000):
    ...
    for complx_sim_res in res_totItr_folders_lst:
        ...
        for indiv_chain_combo_itr_folder in chain_combo_itr_folder_lst:
            ...
            tot_num_itr_executed = tot_replica * itr_per_replica
            print(f'tot_num_itr_executed: {tot_num_itr_executed}')
            
            batch_sim_res_csv_file_lst = glob.glob(os.path.join(chain_combo_itr_res_folder, 'wrapperStepIdx_*.csv'), recursive=False)
            tot_aaccpt_itr = 0
...
if __name__ == '__main__':
    ...
    iteration_tag_lst = ['remc_fullLen_puFalse_mutPrcntLen10']
