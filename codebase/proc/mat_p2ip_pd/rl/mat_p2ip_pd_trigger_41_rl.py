...
from proc.mat_p2ip_pd.rl.mat_p2ip_pd_rl import run_prot_design_simuln
...
def trigger_experiment(root_path = './', user_input_dict=dict()):
    ...
    # ############################ setting inputs for Temperature Scheduler used in RL algorithm  -Start ############################
    # A boolean flag to indicate whether to use temperature scheduler to control the temperature parameter as a part of Metropolis Criterion in Monte Carlo simulation (RL) during protein design through interaction procedure.
    # If this flag is True, then an instance of TemperatureScheduler class is used in RL algorithm and value for the input argument 'fixed_temp_rl' is ignored.
    # If this flag is False then a fixed temperature value (as specified in 'fixed_temp_rl' argument) is used
    def_use_temp_sch_rl = False  # default value
    # check if the user_input_dict contains value for the key 'use_temp_sch_rl'; 
    # otherwise, set it to the default value
    input_dict['use_temp_sch_rl'] = user_input_dict.get('use_temp_sch_rl', def_use_temp_sch_rl)

    # The fixed temperature value used as a part of Metropolis Criterion in Monte Carlo simulation (RL). If 'use_temp_sch_rl' argument is True, then this parameter value is ignored in favor of an instance of TemperatureScheduler class.
    def_fixed_temp_rl = 0.2  # default value
    # check if the user_input_dict contains value for the key 'fixed_temp_rl'; 
    # otherwise, set it to the default value
    input_dict['fixed_temp_rl'] = user_input_dict.get('fixed_temp_rl', def_fixed_temp_rl)
    ...
    # ############################ setting inputs for Temperature Scheduler used in RL algorithm  -End ############################
            ...
            for num_of_itr in input_dict['num_of_itr_lst']:
                for percent_len_for_calc_mut_pts in input_dict['percent_len_for_calc_mut_pts_lst']:
                    ...
                    run_prot_design_simuln(root_path=input_dict['root_path'], dim_prot_complx_nm=dim_prot_complx_nm, use_prot_unit=input_dict['use_prot_unit'], num_of_itr=num_of_itr
                                        ...
                                        , fixed_temp_rl=input_dict['fixed_temp_rl'], temperature_scheduler=temperatureScheduler_inst
...
if __name__ == '__main__':
    ...
    # ###################################################### #
    # ###################################################### #
    # dim_prot_complx_nm_lst_41_orig = ['2I25']
    dim_prot_complx_nm_lst_41_orig = ['1ZHH', '1ZHI', '2A5T', '2AJF', '2BTF', '2FJU', '2G77', '2HLE', '2HQS', '2VDB', '3D5S']
    # dim_prot_complx_nm_lst_41_orig = ['2I25', '4Y7M', '5JMO', '5SV3', '6CWG', '6DBG', '1AVX', '1AY7', '1BUH', '1BVN'] \
    ...
    dim_prot_complx_nm_lst_effective = [prot_id for prot_id in dim_prot_complx_nm_lst_41_orig if prot_id not in dim_prot_complx_nm_lst_excluded]
    ...
    # ###################################################### #
    user_input_dict['result_dump_dir'] = os.path.join(root_path, f'dataset/proc_data/result_dump_rl_fullLen_puFalse_batch5_mutPrcntLen10')
    # ###################################################### #
    user_input_dict['fixed_temp_rl'] = 1.0    
    # ###################################################### #
    ...
    user_input_dict['use_temp_sch_rl'] = False
