...
def run_prot_design_simuln_through_rl(**kwargs):
    ...
    """
    print('inside run_prot_design_simuln_through_rl() method - Start')
    print('####################################')
    ...
    fixed_temp_rl = kwargs.get('fixed_temp_rl')
    ...
    # ################ for unique_chain_name_tuple_idx in range(1,len(unique_chain_name_tuples_lst)): ################ TEMP CODE FOR AD-HOC TESTING
        ...
        t1 = time.time()
        mut_idx = 0
        # initialize prv_best_mutated_prot_seq (This sequence can be either the previous mutated sequence with maximum PPI probability or the last RL satifying mutated sequence).
        prv_best_mutated_prot_seq = mut_prot_seq
        # initialize p_stable
        p_stable = -1.0
        # temperature used in the RL (Monte Carlo simulation with Metropolis Criteria)
        # It is initiated with 'fixed_temp_rl' value and later is updated by temperature scheduler (if temperatureScheduler_inst is not None).
        t_rl = fixed_temp_rl
        ...
        for batch_idx in range(num_of_batches):
            ...
            # ############### execute RL (Monte Carlo simulation with Metropolis Criteria) algo -Start ###############
            # p_stable corresponds to e_stable of RL algo. But the main difference is thet, in case
            ...
            # Note that irrepective of whether Metropolis Criteria step in RL is required for the current iteration, this temperature 
            # scheduler instance needs to be called to keep track of the scheduling
            if(temperatureScheduler_inst_dcopy != None):
                t_rl = temperatureScheduler_inst_dcopy.adjust_temperature(p_i)
            ...
            if(p_i > p_stable):
                ...
                print(f'rand: {rand} :: p_i: {p_i} :: p_stable: {p_stable} : del_p: {del_p} : exp: {math.exp(-(del_p / t_rl))}')

                if(rand < math.exp(-(del_p / t_rl))):  # Metropolis Criteria
            ...
            t8 = time.time()
            print(f'{fix_mut_prot_id_tag}batch_idx={batch_idx} :: RL :: time (in sec) for execution = {round(t8-t7, 3)}')
            # ############### execute RL (Monte Carlo simulation with Metropolis Criteria) algo -End ###############
    ...
    print('inside run_prot_design_simuln_through_rl() method - End')
